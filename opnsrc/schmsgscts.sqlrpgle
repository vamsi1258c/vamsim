...
SGOP   // yajl variables
SGOP  /include yajl/QRPGLESRC,yajl_h
...
SGOA d*PROD_NAME        c                   'SCHMSGSCGI'
SGOA dPROD_NAME        c                   'SCHMSGSCTS'
     ...
     dhtmlErrPos       s              2a   inz
       ...
       httpAccept = getHttpAccept();                         //SG
SGOP   httpAccept = 'json';
...
SGOP   // Code to generate Open API Specifications
SGOP   // Save Request data in a file
SGOP     saveReqData(@appl_name
SGOP                 :hscrn_type
SGOP                 :h_action
SGOP                 :content_type
SGOP                 :htmlform
SGOP                 :bufin);
     ...
     dmsg              ds                  likeds(msgDs_t) inz
       ...
       if varType = 'MSG';
          ...
          when %subst(variable:1:5) = 'ERROR';
             ...
             //SG  if errs_sect(erridx) <> htmlErrPos;
             //SG     iter;
             //SG  endif;
          ...
          //Escape special characters                           //SG
          if httpAccept = 'json';                               //SG
             returnVal = sanitizeJSONResp(returnVal);           //SG
          endif;                                                //SG
       ...
       if varType = 'VAR';
          ...
          when variable = 'CATGRY';                 //SG - For SELECTED
             returnVal = hcategry;                  //SG
     ...
     dworkbuffer       s                   like(smallBuffer)
      ...
      /include yajl/QRPGLESRC,yajl_h
       ...
       // *********************************************************************
       // sanitizeJSONResp: Escape special characters in JSON response
       // *********************************************************************
...
SGOP   // *********************************************************************
SGOP   // saveReqData: Save Request Data in a file for Open API Spec generation
SGOP   //  parms
SGOP   //    inApp     -  Program Name
SGOP   //    inScrn    -  Screen Id
SGOP   //    inAction  -  User Action
SGOP   //    inContent -  Content Type: text/json or key value pairs
SGOP   //    inForm    -  Request data in Key/value pairs
SGOP   //    inData    -  Request data as came in (used in case of JSON type)
SGOP   // *********************************************************************
SGOP   dcl-proc saveReqData;
SGOP      dcl-pi *n;
SGOP         inApp      like(@appl_name)   value;
SGOP         inScrn     like(hscrn_type)   value;
SGOP         inAction   like(h_action)     value;
SGOP         inContent  like(content_type) value;
SGOP         inForm     likeds(htmlForm)   value;
SGOP         inData     like(bufin)        value;
SGOP      end-pi;
SGOP      dcl-s wReqId char(30);
SGOP      dcl-s idx int(5) inz;

SGOP      // if the request is not in json format, then convert it into JSON format
SGOP      if %trim(inContent) <> 'text/json';
SGOP
SGOP         // begin JSON object
SGOP         yajl_genOpen(*off);
SGOP         yajl_beginObj();
SGOP
SGOP         // loop through key value pairs and add to JSON string
SGOP         for idx = 1 to inForm.numInputs;
SGOP            yajl_addChar(%trim(inForm.input(idx).name)
SGOP                        :%trim(inForm.input(idx).data));
SGOP         endfor;
SGOP
SGOP         // end JSON object
SGOP         yajl_endObj();
SGOP
SGOP         // Copy the result from memory
SGOP         inData = yajl_copyBufStr();
SGOP
SGOP         yajl_genClose();
SGOP      endif;
SGOP
SGOP      wReqId = %trim(inScrn)+'_'+%trim(inAction);
SGOP
SGOP      // delete any existing data
SGOP      exec sql
SGOP        delete from UTOASLOG
SGOP            where rr_appnm = :inApp and rr_reqid = :wreqid;
SGOP
SGOP      exec sql
SGOP         insert into UTOASLOG
SGOP            values(' ', :inApp, :wReqId, 'REQ',  1, :inData);
SGOP
SGOP   end-proc;

     ...
     dhtmlErrPos       s              2a   inz
       ...
       if hscrn_next = '303'; // redirected to a different URL
          ...
          exsr setrootpath;
SGTST     panelFile = %scanRpl('.html':'.json':panelFile);    //SG Test
...
SGTSTd firstIteration  s               N   Inz(*On)                             // SG Test
SGTSTd comma           C                   ','                                  // SG Test
       ...
       when criteria = 'SECTIONS';
          ...
          for sectIdx=1 to sect_size;
             ...
             if startIdx > 0;
SGTST           workbuffer = %trim(template);
SGTST           if firstIteration;
SGTST             firstIteration = *Off;                                       // SG test Code
SGTST           elseIf %scan(DOT_JSON:panelFile) > 0;
SGTST             workbuffer = comma+%trim(template);
SGTST           endif;
SGTST  //       web_replaceVariables(template:
SGTST           web_replaceVariables(workbuffer:
                                     %trim(rootFilePath):
                                     %paddr('GETVARVALUES'):
                                     %paddr('PROCESSIF'):
                                     %paddr('PROCESSLOOP'):
